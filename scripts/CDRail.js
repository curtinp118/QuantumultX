...
*******************************
[rewrite_local]
^https:\/\/app\.cdmetro\.chengdurail\.cn\/platform\/users\/bus\/check-payment-notify$ url script-request-header https://raw.githubusercontent.com/curtinp118/QuantumultX/refs/heads/main/scripts/CDRail.js
...
if (isGetHeader) {
  ...
  if (!picked || Object.keys(picked).length === 0) {
    ...
    $notify("成都地铁 Headers 获取失败", "", "未获取到指定请求头，请重新触发一次抓包 URL。");
    $done({});
  } else {
  ...
  if (!raw) {
    $notify("成都地铁签到结果", "无法签到", "本地没有已保存的请求头，请先抓包访问一次 check-payment-notify 接口。");
    return $done();
  }
